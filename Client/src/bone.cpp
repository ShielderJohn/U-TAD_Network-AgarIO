...
void Bone::Render() {
	...
	if (image != NULL) {
		...
		Renderer::Instance().DrawImage(image, 0, 0, 0, currentScaleX * image->GetWidth(), currentScaleY * image->GetHeight());
		Renderer::Instance().TranslateMatrix(pivotX * image->GetWidth(), pivotY * image->GetHeight(), 0);
	}
